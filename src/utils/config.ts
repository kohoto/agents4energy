...
export const defaultAgents: { [key: string]: BaseAgent | BedrockAgent | LangGraphAgent } = {
    PlanAndExecuteAgent: {
        ...
        samplePrompts: [
            '「ピグ作業」について教えて',
            '設備管理番号30-045-29202で、ガス漏れが発生しました。対応手順を教えて',
            '設備管理番号30-045-29202で今まで発生したイベントを知りたい',
            '設備管理番号30-045-29202のガス生産量が知りたい',
            // 'API番号30-045-29202の井戸のパイプラインに異変があったため、操業を停止しました。今までどんなイベントがあったか教えて。'.replace(/^\s+/gm, ''),
            ...
            // `.replace(/^\s+/gm, ''),
            // `今までにこの坑井ではどんなイベントがあったのか教えて`.replace(/^\s+/gm, ''), //This trims the white space at the start of each line
